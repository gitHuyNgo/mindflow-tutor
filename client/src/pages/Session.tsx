...
const SessionPage = () => {
  ...
  // Click to start recording, click again to stop & send via OpenAI STT
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    if (awaitingDistractionReplyRef.current) {
      ...
      // Not an ack → resume detector, process as normal question below
      detectorPausedRef.current = false;
      setDetectorState(null);
    }
    ...
    if (awaitingConfusionReplyRef.current) {
      ...
      try {
        const r = await fetch("/api/v1/utils/classify-intent", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ text, intent: "the user is confirming yes they are confused or need help" }),
        });
        isYes = (await r.json()).match === true;
      } catch { /* fallback */ }
      ...
      // User denied or said something else → discard frame, resume detector
      detectorPausedRef.current = false;
      setDetectorState(null);
      pendingConfusionFrameRef.current = null;
      // If it's a denial (no/nope/not really) → swallow, don't send to AI
      let isDenial = false;
      try {
        const r = await fetch("/api/v1/utils/classify-intent", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ text, intent: "the user is saying no they are not confused" }),
        });
        isDenial = (await r.json()).match === true;
      } catch { /* fallback */ }
      if (isDenial) return;
      // Otherwise fall through to normal question
    }

    // ── Understanding confirmation during confusion interaction ──
    if (inConfusionInteractionRef.current) {
      ...
      try {
        const r = await fetch("/api/v1/utils/classify-intent", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ text, intent: "the user fully understands the topic and has no more questions about it" }),
        });
    ...
    try {
      ...
      if (confusionFrameReadyRef.current) {
        // First question after confirming confusion — send captured frame as context
  ...
  // ── Camera detector polling — every 2.5s when cam is on ──
  useEffect(() => {
      ...
      try {
          ...
          <div id="pip-root" style="display:flex;flex-direction:column;height:100dvh;overflow:hidden">
            <div style="height:240px;position:relative;flex-shrink:0;background:#000;overflow:hidden">
              <video id="pip-cam" autoplay playsinline muted
                style="width:100%;height:100%;object-fit:cover;object-position:center top;transform:scaleX(-1);display:block"></video>
            ...
            <div id="pip-chat" style="flex:1;overflow-y:auto;padding:10px 11px 6px;display:flex;flex-direction:column;gap:8px;min-height:0"></div>
            <div style="padding:7px 10px;display:flex;gap:6px;align-items:center;border-top:1px solid rgba(255,255,255,.06)">
              <input id="pip-input" type="text" placeholder="Ask anything..."
                style="flex:1;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.09);transition:border-color .15s,background .15s;border-radius:24px;padding:7px 14px;color:white;font-size:12px;font-family:'DM Sans',sans-serif">
              <button id="pip-stt" class="pip-btn" style="width:32px;height:32px;border-radius:50%;border:none;background:rgba(255,255,255,.07);color:rgba(255,255,255,.5)">
                <span class="ms" style="font-size:17px">mic</span>
              </button>
              <button id="pip-send" class="pip-btn" style="width:32px;height:32px;border-radius:50%;border:none;background:rgba(138,180,248,.85);color:#030D1A;box-shadow:0 0 12px rgba(138,180,248,.2)">
            ...
            <div style="padding:8px 12px 13px;display:flex;align-items:center;gap:8px;border-top:1px solid rgba(255,255,255,.06);flex-shrink:0">
              ...
              <button id="pip-end" class="pip-btn" style="width:38px;height:38px;border-radius:50%;border:none;background:rgba(239,68,68,.85);color:white;box-shadow:0 0 12px rgba(239,68,68,.25)">
        ...
        const renderMsg = (m: Message) => {
          ...
          return `<div style="display:flex;gap:6px;justify-content:${isUser ? "flex-end" : "flex-start"};align-items:flex-end">
            ...
            <div style="max-width:83%;background:${isUser ? "rgba(138,180,248,.2)" : "rgba(138,180,248,.06)"};border:1px solid ${isUser ? "rgba(138,180,248,.32)" : "rgba(138,180,248,.15)"};color:rgba(255,255,255,${isUser ? ".95" : ".9"});border-radius:18px;padding:6px 11px;font-size:14.5px;line-height:1.55;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;letter-spacing:0.01em;border-bottom-${isUser ? "right" : "left"}-radius:4px${isUser ? ";box-shadow:0 2px 10px rgba(138,180,248,.1)" : ""}">
              ${imgHtml}${m.content}
            </div>
          </div>`;
        };
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden"
      ...
      <header className="nav-glass shrink-0 flex items-center gap-3 px-4 md:px-6 h-[3.4rem] z-10 border-b border-white/[0.06]">
        ...
        <button onClick={isPiP ? closePiP : openPiP}
          className={`flex items-center gap-1.5 text-[11px] px-2.5 py-1.5 rounded-lg transition-all cursor-pointer ${
            isPiP ? "bg-primary/20 text-primary hover:bg-primary/30 border border-primary/20" : "text-white/40 hover:text-white/75 hover:bg-white/8 border border-transparent"
          }`} style={{ fontFamily: "'DM Sans', sans-serif" }}>
          {isPiP ? "↩ Return" : "⧉ Float"}
        </button>
          ...
          <div className="max-w-2xl mx-auto space-y-4">
            <AnimatePresence initial={false}>
              {messages.map(msg => (
                <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}
                  ...
                  <div className={`max-w-[72%] rounded-[18px] px-4 py-2.5 text-[16px] leading-relaxed shadow-sm ${
                    msg.role === "user"
            ...
            {isLoading && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex gap-3 justify-start">
                ...
                <div className="bg-[rgba(138,180,248,0.07)] border border-[rgba(138,180,248,0.18)] rounded-[18px] rounded-bl-[4px] px-5 py-3.5 flex gap-1.5 items-center">
                  {[0, 130, 260].map(d => (
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setCamOn(p => !p)} title={camOn ? "Turn off camera" : "Turn on camera"}
              className={`w-9 h-9 rounded-full flex items-center justify-center transition-all active:scale-90 cursor-pointer ${
                camOn ? "bg-[rgba(138,180,248,0.85)] text-[#030D1A] shadow-[0_0_12px_rgba(138,180,248,0.25)]" : "bg-red-500/12 text-red-400 hover:bg-red-500/22 border border-red-500/20"
              }`}>
              {camOn ? <Video className="w-4 h-4" /> : <VideoOff className="w-4 h-4" />}
            </button>
            <button onClick={() => setAudioOn(p => !p)} title={audioOn ? "Mute AI voice" : "Unmute AI voice"}
              className={`w-9 h-9 rounded-full flex items-center justify-center transition-all active:scale-90 cursor-pointer ${
                audioOn ? "bg-[rgba(138,180,248,0.85)] text-[#030D1A] shadow-[0_0_12px_rgba(138,180,248,0.25)]" : "bg-red-500/12 text-red-400 hover:bg-red-500/22 border border-red-500/20"
              }`}>
              {audioOn ? <Volume2 className="w-4 h-4" /> : <VolumeX className="w-4 h-4" />}
            </button>
            <button onClick={switchScreen} title={screenOn ? "Switch screen" : "Share screen"}
              className={`w-9 h-9 rounded-full flex items-center justify-center transition-all active:scale-90 cursor-pointer ${
                screenOn ? "bg-[rgba(138,180,248,0.85)] text-[#030D1A] shadow-[0_0_12px_rgba(138,180,248,0.25)]" : "bg-white/[0.06] text-white/50 hover:text-white/80 hover:bg-white/[0.09] border border-white/[0.09]"
              }`}>
            ...
            <button onClick={() => analyzeScreen()} disabled={isLoading || !screenStreamRef.current}
              ...
              style={{ fontFamily: "'DM Sans', sans-serif" }}>
              <Scan className="w-3.5 h-3.5" />Analyze Screen
            </button>
            <div className="flex-1" />
            <div className="flex items-center gap-1">
              ...
              <button onClick={() => navigate("/login")}
                ...
                style={{ fontFamily: "'DM Sans', sans-serif" }}>Sign Up</button>
            </div>
          </div>
          ...
          <div className="flex gap-2 items-center">
            ...
            <button onClick={toggleRecording} title={isRecording ? "Stop recording" : "Speak to AI"}
              className={`w-11 h-11 rounded-full flex items-center justify-center transition-all active:scale-95 cursor-pointer ${
                isRecording ? "bg-red-500 text-white shadow-[0_0_0_4px_rgba(239,68,68,0.25)] animate-pulse" : "bg-white/[0.07] border border-white/[0.1] text-white/55 hover:text-white/85 hover:bg-white/[0.12]"
              }`}>
