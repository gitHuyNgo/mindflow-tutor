...
const SessionPage = () => {
  ...
  const addMessage = useCallback((role: "user" | "ai", content: string) => {
    setMessages(prev => {
      const next = [...prev, { id: crypto.randomUUID(), role, content }];
    ...
    if (supportsDocPiP) {
      try {
        ...
        body.innerHTML = `
          <div id="pip-root" style="display:flex;flex-direction:column;height:100dvh;overflow:hidden">
            ...
            <div style="padding:8px 12px 13px;display:flex;align-items:center;gap:8px;border-top:1px solid rgba(255,255,255,.06);flex-shrink:0">
              <button id="pip-mic" class="pip-btn"
        ...
        const updateControls = (mic: boolean, cam: boolean) => {
          ...
          micBtn.style.color = mic ? "#030D1A" : "white";
          micBtn.innerHTML = `<span class="ms">${mic ? "mic" : "mic_off"}</span>`;
      ...
      <div className="shrink-0 border-t border-white/[0.07] bg-[#030D1A]/96 backdrop-blur-md px-6 py-3">
        <div className="max-w-2xl mx-auto space-y-2.5">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setMicOn(p => !p)} title={micOn ? "Mute mic" : "Unmute mic"}
